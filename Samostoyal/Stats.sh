...
# ASCII-only версия для Git-Bash
...
if ! command -v jq >/dev/null 2>&1; then
    echo -e "${RED}Ошибка: утилита 'jq' не установлена.${RESET}"
    echo -e "${YELLOW}В Git-Bash скачайте jq.exe или используйте WSL.${RESET}"
    exit 1
fi
...
case "$HTTP_CODE" in
    200) ;;
    404)
        echo -e "${RED}Репозиторий '$REPO' не найден.${RESET}"
        exit 1
        ;;
    403)
